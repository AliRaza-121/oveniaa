...
export default function HomeClient({ popularItems, categories, stats }) {
  return (
    <div>
     {/* Hero */}
<section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-bg pt-24 sm:pt-28">
  ...
  <div className="absolute inset-0 overflow-hidden">
    <motion.div
      ...
      className="absolute -top-20 -right-20 w-[500px] h-[500px] bg-primary/10 rounded-full blur-[120px]"
    />
    <motion.div
      animate={{ x: [0, -60, 0], y: [0, 50, 0], scale: [1, 1.2, 1] }}
      transition={{ repeat: Infinity, duration: 22, ease: 'linear' }}
      className="absolute -bottom-32 -left-20 w-[400px] h-[400px] bg-secondary/10 rounded-full blur-[100px]"
    />
  </div>
  ...
  <div className="max-w-4xl mx-auto px-4 sm:px-6 w-full relative z-10 text-center">
    ...
    <motion.div
      ...
      className="inline-flex items-center gap-3 bg-primary/10 border border-primary/20 text-primary text-xs font-bold px-5 py-2.5 rounded-full mb-10"
    >
    ...
    <motion.p
      ...
      className="text-text-muted text-base sm:text-lg mt-8 max-w-xl mx-auto leading-relaxed"
    >
      Fresh ingredients. Bold flavors. Delivered hot to your door in{' '}
      <span className="text-primary font-semibold">30 minutes or less</span>.
      That is the Oveniaa promise.
    </motion.p>
  ...
  <Link href="/menu" className="bg-primary text-white px-6 py-3.5 rounded-full font-bold text-sm hover:bg-primary-dark transition-all hover:scale-105 shadow-xl shadow-primary/30 inline-flex items-center gap-2">
    Order Online
  ...
  <a
    href="https://wa.me/923248034267?text=Hi%20Oveniaa!%20I%20want%20to%20place%20an%20order."
    target="_blank"
    className="bg-emerald-500 text-white px-6 py-3.5 rounded-full font-bold text-sm hover:bg-emerald-600 transition-all hover:scale-105 shadow-xl shadow-emerald-500/30 inline-flex items-center gap-2"
  >
    ...
    <div className="flex gap-2 overflow-x-auto scrollbar-hide sm:justify-center sm:gap-3">
      {categories.map(cat => (
        <Link
          ...
          className="flex items-center gap-1.5 bg-card border border-border rounded-full px-3.5 py-2 sm:px-5 sm:py-2.5 text-xs sm:text-sm font-semibold text-text-light hover:text-primary hover:border-primary whitespace-nowrap transition-all flex-shrink-0"
        >
          <span className="text-sm sm:text-lg">
            {cat.name === 'Burgers' ? '🍔' : cat.name === 'Pizzas' ? '🍕' : cat.name === 'Fries & Sides' ? '🍟' : '🥤'}
          </span>
          {cat.name}
        </Link>
      ))}
...
<section className="py-16">
  ...
  <img 
    src={popularItems[0].image.replace('/upload/', '/upload/w_400,f_auto,q_auto/')} 
    alt="" 
  ...
  <span>🍔</span>
        ...
        {/* Cards — Grid on desktop, scroll on mobile */}
  ...
  <span>{item.category === 'Burgers' ? '🍔' : item.category === 'Pizzas' ? '🍕' : item.category === 'Fries & Sides' ? '🍟' : '🥤'}</span>
              ...
              <div className="flex items-center gap-1 mt-1">
                <span className="text-yellow-500 text-xs">{'★'.repeat(item.avgRating || 0)}{'☆'.repeat(5 - (item.avgRating || 0))}</span>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[1,2,3,4].map(i => (
            <div key={i} className="animate-pulse">
              <div className="h-32 bg-card rounded-xl mb-3" />
    ...
    <div className="flex flex-col lg:flex-row gap-8 items-center">
      ...
      <div className="lg:w-80 flex flex-col items-center gap-4">
        <a
          ...
          target="_blank"
          className="bg-primary text-white px-8 py-4 rounded-full font-bold text-base hover:bg-primary-dark transition-all hover:scale-105 shadow-xl shadow-primary/30 inline-flex items-center gap-2"
        >
          🗺️ Get Directions
        ...
        <a href="tel:+923248034267" className="text-text-muted hover:text-primary transition-colors text-sm font-medium">
          📞 +92 324 8034267
        </a>
      </div>
    </div>
