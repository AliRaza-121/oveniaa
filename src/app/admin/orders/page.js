      ...
      <div className="space-y-3 sm:space-y-4">
        {filtered.map(order => (
          <div key={order._id} className="bg-card border border-border rounded-2xl p-4 sm:p-5">
            <div className="flex justify-between items-start mb-3">
              <div className="min-w-0 flex-1 mr-3">
                ...
                <p className="text-[10px] sm:text-xs text-text-muted mt-1">
                  🕐 {new Date(order.createdAt).toLocaleTimeString('en-PK', { hour: '2-digit', minute: '2-digit' })} • {Math.round((Date.now() - new Date(order.createdAt).getTime()) / 60000)} min
                </p>
              </div>
            ...
            <div className="text-[10px] sm:text-sm text-text-muted mb-3 bg-bg rounded-xl p-2 sm:p-3">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
              ...
              <button onClick={() => updateStatus(order._id, activeTab === 'pending' ? 'confirmed' : activeTab === 'confirmed' ? 'preparing' : activeTab === 'preparing' ? 'ready' : 'delivered')} className="text-[10px] sm:text-xs bg-primary text-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-full font-medium hover:bg-primary-dark transition-colors">
                Next →
              </button>
