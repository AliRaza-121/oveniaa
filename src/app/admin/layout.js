...
export default function AdminLayout({ children }) {
  ...
  if (!user || user.role !== 'admin') {
    return <div className="min-h-screen flex items-center justify-center px-4"><div className="text-center"><span className="text-5xl">🔒</span><p className="text-text-muted mt-4 text-sm">Access denied</p><Link href="/" className="text-primary mt-2 inline-block hover:underline text-sm">Go Home</Link></div></div>
  }
  ...
  return (
    <div className="min-h-screen bg-bg flex">
      ...
      <motion.aside animate={{ width: collapsed ? 72 : 240 }} className={`bg-card border-r border-border flex flex-col fixed left-0 top-0 bottom-0 z-40 overflow-hidden transition-transform ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
      ...
      <main className={`flex-1 transition-all duration-300 ${collapsed ? 'md:ml-[72px]' : 'md:ml-[240px]'} ml-0 min-h-screen`}>
        <div className="h-14 border-b border-border bg-card flex items-center justify-between px-3 sm:px-6 sticky top-0 z-20 gap-2 sm:gap-3">
          ...
          <form onSubmit={handleSearch} className="flex-1 max-w-md hidden sm:block">
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search orders..." className="w-full bg-bg border border-border rounded-full px-4 py-1.5 text-xs text-text focus:outline-none focus:border-primary" />
          </form>
